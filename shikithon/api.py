# pylint: disable=W0311 (bad indentation)
import json
...
class API:
    ...
    def get(
        ...
        if response.status_code == 401:
            if response_data["error"] == "invalid_token":
                self.update_tokens(
                    self.get_tokens_from_api(refresh_tokens=True)
                )
    ...
    def export_api_config(self, file_name: str = "new_config.json"):
        ...
        with open(file_name, "w", encoding="utf-8") as config_file:
            json.dump(
                current_config, config_file, indent=4, separators=(", ", ": ")
            )
    ...
    def api_vars_validation(self):
        if not self.app_name:
            raise MissingConfigData(
                "To use the Shikimori API correctly, " \
                "you need to pass the application name"
            )
        ...
        if not self.auth_code:
            oauth_url_data: Union[str, List[str]] = self.endpoints.get_filled_oauth_url(  # pylint: disable=C0301 (line-too-long)
    ...
    def raise_config_mismatch(self, api_config):
        configs_keys: Tuple[List[str], List[str]] = list(
            api_config.keys()
        ), list(
            APIHelpers.get_blank_config().keys()
        )
        missing_keys_str: str = ", ".join(
            APIHelpers.get_missing_keys_list(configs_keys)
        )
        raise MissingConfigData(
            "It is impossible to initialize an object " \
            "without missing variables. Here is the list " \
            f"of missing: {missing_keys_str}"
        )
    ...
    def get_tokens_from_api(
        ...
        try:
            new_tokens = oauth_json["access_token"], oauth_json["refresh_token"]
            self.session.headers.update(
    ...
    def parse_oauth_url_data(self, oauth_url_data):
        if isinstance(oauth_url_data, list):
            ...
            return "The authorization code and some related " \
                "information necessary to obtain it are missing: " \
                f'"{missing_variables_str}".' \
                f"\nGo to {self.endpoints.get_oauth_url()} " \
                "for more information to get the authorization code"
        return "The authorization code is missing. " \
            f"It's okay, just paste this link \"{oauth_url_data}\" " \
            "and then save the authorization code in your configuration"

    def get_achievements(self, user_id: int) -> List[Achievement]:
        query: Dict[str, str] = {
            "user_id": str(user_id)
        }
        res: List[Dict[str, Any]] = self.get(
            url=self.endpoints.get_achievements_url(), query=query
        )
    ...
    def get_list_of_animes(
        ...
        """
            ...
            Parameters:
                ...
                genre (List[int] | None): IDs of genres
                studio (List[int] | None): IDs of studios
                franchise (List[int] | None): IDs of franchises
                censored (Censorship): type of anime censorship
                my_list (MyList): status of anime in current user list
                ids (List[int] | None): IDs of animes to include
                excluded_ids (List[int] | None): IDs of animes to exclude
        ...
        res: List[Dict[str, Any]] = self.get(
            url=self.endpoints.get_animes_url(), query=query
        )
    ...
    def get_anime(self, anime_id: int) -> Anime:
        ...
        """
        res: Dict[str, Any] = self.get(
            url=self.endpoints.get_anime_url(anime_id)
        )
        return Anime(**res)
    ...
    def get_anime_creators(self, anime_id: int) -> List[Creator]:
        ...
        """
        res: List[Dict[str, Any]] = self.get(
            url=self.endpoints.get_anime_roles_url(anime_id)
        )
    ...
    def get_list_of_similar_animes(self, anime_id: int) -> List[Anime]:
        ...
        """
        res: List[Dict[str, Any]] = self.get(
            url=self.endpoints.get_similar_animes_url(anime_id)
        )
        return [Anime(**anime) for anime in res]
    ...
    def get_anime_related_content(self, anime_id: int) -> List[Relation]:
        ...
        """
        res: List[Dict[str, Any]] = self.get(
            url=self.endpoints.get_anime_related_content_url(anime_id)
        )
    ...
    def get_anime_screenshots(self, anime_id: int) -> List[Screenshot]:
        ...
        """
        res: List[Dict[str, Any]] = self.get(
            url=self.endpoints.get_anime_screenshots_url(anime_id)
        )
    ...
    def get_anime_franchise_tree(self, anime_id: int) -> FranchiseTree:
        ...
        """
        res: Dict[str, Any] = self.get(
            url=self.endpoints.get_anime_franchise_tree_url(anime_id)
        )
    ...
    def get_anime_external_links(self, anime_id: int) -> List[Link]:
        ...
        """
        res: List[Dict[str, Any]] = self.get(
            url=self.endpoints.get_anime_external_links_url(anime_id)
        )
    ...
    def get_anime_topics(
        ...
        res: List[Dict[str, Any]] = self.get(
            url=self.endpoints.get_anime_topics_url(anime_id), query=query
        )
    ...
    def get_bans_list(self, page: int = 1, limit: int = 1) -> list[Ban]:
        ...
        res: List[Dict[str, Any]] = self.get(
            url=self.endpoints.get_bans_list_url(), query=query
        )
    ...
    def get_current_calendar(
        ...
        """
            ...
            Parameters:
                censored (Censorship): Status of censorship
                (Defaults to Censorship.CENSORED)
        ...
        res: List[Dict[str, Any]] = self.get(
            url=self.endpoints.get_calendar_url(), query=query
        )
...
class APIHelpers:
    @staticmethod
    def get_missing_keys_list(
        configs_tuple: Tuple[List[str], List[str]]
    ) -> List[str]:
